...
class DataPreprocessor:
    def __init__(self, target_column=None, test_size=0.2, random_state=42):
        ...
        self.problem_type = (
            None  # 'binary_classification', 'multiclass_classification', 'regression'
        )
    ...
    def create_features(self, df):
        ...
        gpa_cols = [
            col
            for col in numeric_cols
            if any(term in col.lower() for term in ["grade", "gpa", "score"])
        ]
    ...
    def encode_target(self, y):
        ...
        if (
            unique_ratio > 0.1
        ):  # Likely regression - too many unique values to be classification
    ...
    def split_data(self, X, y, stratify=None):
        ...
        if stratify is None:
            ...
            stratify = (
                unique_ratio < 0.1
            )  # Use stratification if less than 10% unique values
    ...
    def fit_transform(self, X_train, X_val, X_test):
        ...
        if self.preprocessor is None:
            raise ValueError(
                "Preprocessor not initialized. Call prepare_features first."
            )
    ...
    def _get_feature_names(self):
        ...
        try:
            ...
            num_features = (
                len(self.numeric_features) if hasattr(self, "numeric_features") else 0
            )
            num_cat_features = (
            ...
            feature_names = [
                f"feature_{i}" for i in range(num_features + num_cat_features)
            ]
