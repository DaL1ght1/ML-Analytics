...
class ModelTrainer:
    ...
    def _initialize_models(self):
        if self.is_regression:
            self.models = {
                ...
                "Ridge Regression": Ridge(random_state=self.random_state, alpha=1.0),
                "Lasso Regression": Lasso(
                    random_state=self.random_state, alpha=1.0, max_iter=1000
                ),
            ...
            self.models = {
                ...
                "SVM": SVC(
                    random_state=self.random_state, probability=True, kernel="rbf"
                ),
    ...
    def train_models(
        ...
        for model_name, model in self.models.items():
            ...
            try:
                ...
                results = {"model": model, "training_time": training_time}
                ...
                if X_val is not None and y_val is not None:
                    ...
                    if self.is_regression:
                        ...
                        try:
                            results["val_mape"] = mean_absolute_percentage_error(
                                y_val, val_pred
                            )
                        except:
                        ...
                        results["val_precision"] = precision_score(
                            y_val, val_pred, average="weighted", zero_division=0
                        )
                        results["val_recall"] = recall_score(
                            y_val, val_pred, average="weighted", zero_division=0
                        )
                        results["val_f1"] = f1_score(
                            y_val, val_pred, average="weighted", zero_division=0
                        )
                ...
                if X_test is not None and y_test is not None:
                    ...
                    if self.is_regression:
                        ...
                        try:
                            results["test_mape"] = mean_absolute_percentage_error(
                                y_test, test_pred
                            )
                        except:
                ...
                cv_scores = cross_val_score(
                    model, X_train, y_train, cv=5, scoring=cv_scoring
                )
    ...
    def evaluate_model(self, model_name, X_test, y_test):
        ...
        try:
            if len(np.unique(y_test)) == 2:  # Binary classification
                evaluation["roc_auc"] = roc_auc_score(
                    y_test, y_pred_proba[:, 1] if y_pred_proba is not None else y_pred
                )
    ...
    def get_best_model(self, metric=None):
        ...
        best_score = -float(
            "inf"
        )  # Use negative infinity for both high-is-better and low-is-better metrics
        ...
        if is_minimize:
            best_score = float(
                "inf"
            )  # Start with positive infinity for minimize metrics
    ...
    def get_feature_importance(self, model_name, feature_names=None, top_n=20):
        ...
        if not hasattr(model, "feature_importances_"):
            return pd.DataFrame(
                {"message": [f"{model_name} does not support feature importance"]}
            )
    ...
    def save_models(self, filepath_prefix="model"):
        ...
        for model_name, model in self.trained_models.items():
            filename = (
                f"{filepath_prefix}_{model_name.replace(' ', '_').lower()}.joblib"
            )
...
def quick_train_models(
    X_train, y_train, X_val=None, y_val=None, X_test=None, y_test=None, random_state=42
):
