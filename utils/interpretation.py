...
class ModelInterpreter:
        ...
        try:
            ...
            if any(
                ...
                try:
                    ...
                    st.warning(
                        f"TreeExplainer failed for {model_name}, using KernelExplainer: {tree_error}"
                    )
                ...
                try:
                    ...
                    st.warning(
                        f"LinearExplainer failed for {model_name}, using KernelExplainer: {linear_error}"
                    )
                    self.shap_explainer = shap.KernelExplainer(
                        (
                            self.model.predict_proba
                ...
                self.shap_explainer = shap.KernelExplainer(
                    ...
                    shap.sample(
                        self.X_train, min(50, len(self.X_train))
                    ),  # Use smaller sample for better performance
                )
            ...
            if self.shap_explainer is not None:
                ...
                try:
                    ...
                    st.warning(
                        f"SHAP explainer test failed for {model_name}: {test_error}"
                    )
                    self.shap_explainer = None

        except Exception as e:
            st.warning(
                f"Could not initialize SHAP explainer for {type(self.model).__name__}: {str(e)}"
            )
            self.shap_explainer = None
    ...
    def plot_shap_summary(
        self, X_explain, max_samples=100, plot_type="bar", class_idx=None
    ):
        ...
        try:
            ...
            try:
                if plot_type == "bar":
                    ...
                    # Get feature names
                    feature_names = (
                        self.feature_names[: len(feature_importance)]
                ...
                if len(plot_values.shape) == 2:
                    ...
                    feature_importance = (
                        ...
                        else np.mean(np.abs(plot_values), axis=0)
                    )
        ...
        try:
            ...
            if isinstance(shap_values, list):
                ...
                try:
                    ...
                    valid_shap = next(
                        (sv for sv in shap_values if sv is not None and sv.size > 0),
                        None,
                    )
                    if valid_shap is None:
                        return pd.DataFrame()
    ...
    def create_interpretation_dashboard(self, X_explain, sample_idx=0, max_samples=50):
        ...
        if SHAP_AVAILABLE and self.shap_explainer is not None:
            dashboard["shap_importance"] = self.get_shap_feature_importance(
                X_explain, max_samples
            )
            dashboard["shap_summary_plot"] = self.plot_shap_summary(
                X_explain, max_samples, "bar"
            )
        ...
        if (
            LIME_AVAILABLE
            and self.lime_explainer is not None
            and sample_idx < len(X_explain)
        ):
    ...
    def _create_alternative_waterfall(self, X_sample, sample_idx=0):
        ...
        try:
            ...
            bars = ax.barh(
                range(len(sorted_values)), sorted_values, color=colors, alpha=0.7
            )
        ...
        try:
            ...
            if hasattr(self.model, "predict_proba"):
                ...
                if len(decision_scores.shape) == 1:
                    ...
                    confidence_info["confidence_scores"] = np.max(
                        np.abs(decision_scores), axis=1
                    )
...
def create_interpretation_summary(
    interpreter, X_explain, sample_indices=[0], max_samples=50
):
    ...
    if SHAP_AVAILABLE and interpreter.shap_explainer is not None:
        summary["global_importance"] = interpreter.get_shap_feature_importance(
            X_explain, max_samples
        )
    ...
    for idx in sample_indices[:5]:  # Limit to 5 samples
        if idx < len(X_explain):
            dashboard = interpreter.create_interpretation_dashboard(
                X_explain, idx, max_samples
            )
