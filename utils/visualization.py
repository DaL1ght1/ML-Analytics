...
class Visualizer:
    ...
    def plot_roc_curves(
        self, models_data, title="ROC Curves Comparison", use_plotly=True
    ):
        if use_plotly:
            fig = go.Figure()

            for i, (model_name, (y_true, y_pred_proba)) in enumerate(
                models_data.items()
            ):
                try:
                    ...
                    if len(np.unique(y_true)) == 2:
                        ...
                        fig.add_trace(
                            go.Scatter(
                                ...
                                line=dict(
                                    color=self.colors[i % len(self.colors)], width=2
                                ),
                            )
                        )
                    else:
                        ...
                        for class_idx in range(len(classes)):
                            fpr, tpr, _ = roc_curve(
                                y_bin[:, class_idx], y_pred_proba[:, class_idx]
                            )
    ...
    def plot_feature_importance(
        self, importance_df, title="Feature Importance", top_n=20, use_plotly=True
    ):
        ...
        if use_plotly:
            ...
            fig.update_layout(
                height=max(400, top_n * 20), yaxis={"categoryorder": "total ascending"}
            )
            ...
            bars = ax.barh(
                plot_df["feature"], plot_df["importance"], color=self.colors[0]
            )
    ...
    def plot_model_comparison(
        ...
        if metric not in plot_df.columns:
            available_metrics = [
                ...
                if "Accuracy" in col or "F1" in col or "Score" in col
            ]
        ...
        if use_plotly:
            ...
            bars = ax.barh(
                plot_df["Model"], plot_df[metric], color=self.colors[: len(plot_df)]
            )
